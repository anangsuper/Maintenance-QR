...
function render_page(string $title, string $content, string $extraHead = '', string $extraScript = '', bool $showNav = true): void {
    ...
    $navLinks = [
        'OPERATIONS' => [
            ...
            ['title' => 'Kartu Inventaris', 'url' => module_url('dashboard.php', ['tab' => 'kartu']), 'icon' => 'bi-credit-card-2-front', 'active' => ($currentPage === 'dashboard.php' && ($_GET['tab'] ?? '') === 'kartu') || in_array($currentPage, ['inventory_card.php', 'cetak_kartu.php', 'print_inventory_card.php'], true)],
        ],
...
.card, .modal, .dropdown-menu, .dropdown-item, .popover,
.tooltip, .nav, .nav-link, .table, .badge, .badge-chip,
.alert, .toast, .breadcrumb, .pagination, .page-link {
